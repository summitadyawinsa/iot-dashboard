...
$currentDate = date('Y-m-d');
        ...
        <div class="panel h-full sm:col-span-12 xl:col-span-12">
          <div class="grid grid-cols-1 gap-4 p-5 dark:text-white-light">
            <div class="grid grid-cols-3">
              <span class="text-xl font-bold">Shipment Total vs Ship Complete</span>
              <span></span>
          ...
          <div>
            <div class="h-full sm:col-span-6 xl:col-span-2">
              <button id="btn_update_chart"
                x-on:click="() => ChartRecall1(inpt_data_date.value, inpt_data_total_shipment.value, inpt_data_ship_complete.value)"
                hidden>Recall and Update
              </button>
              <input type="text" id="inpt_data_date" value="" hidden>
              <input type="text" id="inpt_data_total_shipment" value="" hidden>
              <input type="text" id="inpt_data_ship_complete" value="" hidden>
        ...
        <div class="panel h-full col-span-12 ">
          <div class="grid grid-cols-1 gap-4 p-5 dark:text-white-light">
            <div class="grid grid-cols-3">
              <label for="select_month" class="text-xl font-bold">CGR Dashboard</label>
              <span></span>
            ...
            <table id="received_project_table" class="min-w-full rounded-md shadow-md overflow-hidden">
              <thead>
                <tr class="bg-gray-600 text-white text-sm leading-normal" style="font-size: 0.9674rem;">
                  ...
                  <th class="py-3 px-6 text-left">Ship Head PackNum</th>
                  <th class="py-3 px-6 text-left">Ship Head LegalNumber</th>
                  <th class="py-3 px-6 text-left">Ship Date</th>
                  <th class="py-3 px-6 text-left">Ship Complete</th>
                  <th class="py-3 px-6 text-left">CGR</th>
                </tr>
              </thead>
            </table>
          </div>
    ...
    document.addEventListener("DOMContentLoaded", function () {
      ...
      function detail_table() {
        detailTable = $("#received_project_table").DataTable({
          ...
          ajax: {
            url: `https://${window.location.host}/api/delivery/get_data_gcr_monitoring_table`,
            type: 'POST',
            contentType: "application/json",
            data: function (d) {
              let selectedDate = document.getElementById('select_month').value;
              if (selectedDate) {
                d.month_select = selectedDate;
              }
          ...
          {
            data: 'ShipHead_PackNum',
          },
          {
            data: 'ShipHead_LegalNumber',
          },
          {
            data: 'ShipDate',
          },
      ...
      function setDefaultData() {
        ...
        const apiUrlDaily = `https://${currentHost}/api/delivery/get_data_monitoring_cgr/${years}`;
        axios.get(apiUrlDaily)
          .then(response => {
            var data_date = response.data.data_date;
            var data_total_shipment = response.data.data_total_shipment;
            var data_shipmemt_complete = response.data.data_shipmemt_complete;
            document.getElementById('inpt_data_date').value = data_date;
            document.getElementById('inpt_data_total_shipment').value = data_total_shipment;
            document.getElementById('inpt_data_ship_complete').value = data_shipmemt_complete;
      ...
      function updateDailyChart() {
        var currentHost = window.location.host;
        var years = document.getElementById('select_years').value;
        const apiUrlMonthly = `https://${currentHost}/api/delivery/get_data_monitoring_cgr/${years}`;
