<x-layout.default>
  ...
  <style>
    #po_ppic_table tbody tr:hover,
    #po_ppic_table_ammount tbody tr:hover,
    #po_approve_table_ppic tbody tr:hover,
    #po_ppic_vendor_table tbody tr:hover,
    #po_ppic_rcv_vs_order tbody tr:hover,
    #po_ppic_table_gr tbody tr:hover {
      background-color: #727e93;
      cursor: pointer;
    }

    #po_ppic_table tbody tr,
    #po_ppic_table_ammount tbody tr,
    #po_ppic_vendor_table tbody tr,
    #po_approve_table_ppic tbody tr,
    #po_ppic_rcv_vs_order tbody tr,
    #po_ppic_table_gr tbody tr {
    ...
    #po_ppic_table thead tr,
    #po_ppic_table_ammount thead tr,
    #po_ppic_vendor_table thead tr,
    #po_approve_table_ppic thead tr,
    #po_ppic_rcv_vs_order thead tr,
    #po_ppic_table_gr thead tr {
      background-color: #4B5563;
      color: whitesmoke;
    }
    ...
    #po_ppic_table_ammount::-webkit-calendar-picker-indicator {
      filter: invert(1);
    }

    select[name="po_ppic_table_length"],
    select[name="po_ppic_table_ammount_length"],
    select[name="po_ppic_vendor_table_length"],
    select[name="po_approve_table_ppic_length"],
    select[name="po_ppic_rcv_vs_order_length"],
    select[name="po_ppic_table_gr_length"] {
      background-color: #4B5563;
      width: 80px;
    }
  </style>
...
$toDate = date('Y-m-d');
        ...
        <div class="panel h-full sm:col-span-6 xl:col-span-6">
          <div class="grid grid-cols-1 gap-4 p-5 dark:text-white-light">
            <div class="grid grid-cols-3">
              <span class="text-xl font-bold">PO PPIC</span>
              <span></span>
            </div>
          </div>
          <hr>
        ...
        <div class="panel h-full sm:col-span-6 xl:col-span-6">
          <div class="grid grid-cols-1 gap-4 p-5 dark:text-white-light">
            <div class="grid grid-cols-3">
              <label for="selectMonth" class="text-xl font-bold">Receipt PO PPIC</label>
              <span></span>
        ...
        <div class="panel h-full col-span-12 ">
          <div class="grid grid-cols-1 gap-4 p-5 dark:text-white-light">
            <div class="grid grid-cols-3">
              <span class="text-xl font-bold">PO PPIC Table</span>
              <span></span>
            </div>
          </div>
          <hr hidden>
          <div class="relative overflow-hidden pt-5">
            <table id="po_ppic_table_gr" class="min-w-full rounded-md shadow-md overflow-hidden">
              <thead>
        ...
        <div class="panel h-full sm:col-span-6 xl:col-span-6">
          <div class="grid grid-cols-1 gap-4 p-5 dark:text-white-light">
            <div class="grid grid-cols-3">
              <span class="text-xl font-bold">Aging PO PPIC</span>
              <span></span>
            </div>
          </div>
          <hr>
        ...
        <div class="panel h-full sm:col-span-6 xl:col-span-6">
          <div class="grid grid-cols-1 gap-4 p-5 dark:text-white-light">
            <div class="grid grid-cols-3">
              <label for="approval_date" class="text-xl font-bold">PO Approval PPIC</label>
              <span></span>
        ...
        <div class="panel h-full col-span-12 ">
          ...
          <hr hidden>
          <div class="relative overflow-hidden pt-5">
            <table id="po_approve_table_ppic" class="min-w-full rounded-md shadow-md overflow-hidden">
              <thead>
                <tr class="bg-gray-600 text-white text-sm leading-normal" style="font-size: 0.9674rem;">
        ...
        <div class="panel h-full col-span-6 ">
          <div class="grid grid-cols-1 p-5 dark:text-white-light">
            <div class="flex justify-between items-start gap-5 flex-wrap">
              <label class="text-xl font-bold">PO PPIC By Days</label>
              <div class="flex flex-col gap-3">
                <div class="flex items-center gap-3">
          ...
          <hr hidden>
          <div class="relative overflow-hidden pt-5">
            <table id="po_ppic_table" class="min-w-full rounded-md shadow-md overflow-hidden">
              <thead>
                <tr class="bg-gray-600 text-white text-sm leading-normal" style="font-size: 0.9674rem;">
                  <th class="py-3 px-6 text-left">Order Date</th>
                  <th class="py-3 px-6 text-left">PO Line</th>
        ...
        <div class="panel h-full col-span-6 ">
          <div class="grid grid-cols-1 gap-4 p-5 dark:text-white-light">
            <div class="grid grid-cols-2">
              <label for="select_date" class="text-xl font-bold">PO PPIC Ammount Days</label>
          ...
          <hr hidden>
          <div class="relative overflow-hidden pt-5">
            <table id="po_ppic_table_ammount" class="min-w-full rounded-md shadow-md overflow-hidden">
              <thead>
                <tr class="bg-gray-600 text-white text-sm leading-normal" style="font-size: 0.9674rem;">
                  <th class="py-3 px-6 text-left">PO Number</th>
                  <th class="py-3 px-6 text-left">PO Line</th>
  ...
  <script>
    ...
    let detailTablePOPPIC = null;

    document.addEventListener("DOMContentLoaded", function () {
      function detail_table() {
        var detailTable = $("#po_ppic_table").DataTable({
          destroy: true,
          processing: true,
          serverSide: true,
          responsive: false,
          ...
          ajax: {
            url: `https://${window.location.host}/api/purchasing/get_purchase_po_ppic`,
            type: 'POST',
        ...
        buttonContainer.id = 'po_ppic_button_container';

        const downloadButton = document.createElement('button');
        downloadButton.id = 'download_po_ppic_button';
        ...
        const tableWrapper = document.querySelector('#po_ppic_table_wrapper');
        if (tableWrapper) {
          const existingContainer = document.getElementById('po_ppic_button_container');
          if (existingContainer) existingContainer.remove();
          tableWrapper.insertAdjacentElement('afterend', buttonContainer);
        } else {
          console.error('Table wrapper #po_ppic_table_wrapper not found');
        }

        downloadButton.addEventListener('click', function () {
          createDateRangePopup('po_ppic');
        });
      ...
      function detail_data_table_ammount() {
        var detailTableAmmount = $("#po_ppic_table_ammount").DataTable({
          destroy: true,
          processing: true,
          serverSide: true,
          responsive: false,
          ...
          ajax: {
            url: `https://${window.location.host}/api/purchasing/get_purchase_data_po_ppic_ammount`,
            type: 'POST',
            contentType: "application/json",
            data: function (d) {
              let startDate = document.getElementById("start_date").value;
        ...
        buttonContainer.style.gap = '10px';
        buttonContainer.style.marginTop = '10px';
        buttonContainer.id = 'po_ppic_ammount_button_container';

        const downloadButton = document.createElement('button');
        downloadButton.id = 'download_po_ppic_ammount_button';
        downloadButton.className = 'btn btn-primary';
        downloadButton.innerHTML = `
            <svg xmlns="https://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-file-earmark-arrow-down-fill" viewBox="0 0 16 16" style="margin-right: 8px;">
                <path d="M9.293 0H4a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h8a2 2 0 0 0 2-2V4.707A1 1 0 0 0 13.707 4L10 .293A1 1 0 0 0 9.293 0M9.5 3.5v-2l3 3h-2a1 1 0 0 1-1-1m-1 4v3.793l1.146-1.147a.5.5 0 0 1 .708.708l-2 2a.5.5 0 0 1-.708 0l-2-2a.5.5 0 0 1 .708-.708L7.5 11.293V7.5a.5.5 0 0 1 1 0"/>
        ...
        const tableWrapper = document.querySelector('#po_ppic_table_ammount_wrapper');
        if (tableWrapper) {
          const existingContainer = document.getElementById('po_ppic_ammount_button_container');
          if (existingContainer) existingContainer.remove();
          tableWrapper.insertAdjacentElement('afterend', buttonContainer);
        } else {
          console.error('Table wrapper #po_ppic_table_ammount_wrapper not found');
        }

        downloadButton.addEventListener('click', function () {
          createDateRangePopup('po_ppic_ammount');
        });
      ...
      function detail_table_po_ppic() {
        detailTablePOPPIC = $("#po_ppic_table_gr").DataTable({
          destroy: true,
          processing: true,
          serverSide: true,
          responsive: false,
          ...
          ajax: {
            url: `https://${window.location.host}/api/purchasing/get_data_purchasing_po_ppic_table`,
            type: 'POST',
            contentType: "application/json",
            data: function (d) {
        ...
        buttonContainer.style.gap = '10px';
        buttonContainer.style.marginTop = '10px';
        buttonContainer.id = 'po_ppic_gr_button_container';

        const downloadButton = document.createElement('button');
        downloadButton.id = 'download_po_ppic_gr_button';
        downloadButton.className = 'btn btn-primary';
        downloadButton.innerHTML = `
            <svg xmlns="https://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-file-earmark-arrow-down-fill" viewBox="0 0 16 16" style="margin-right: 8px;">
                <path d="M9.293 0H4a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h8a2 2 0 0 0 2-2V4.707A1 1 0 0 0 13.707 4L10 .293A1 1 0 0 0 9.293 0M9.5 3.5v-2l3 3h-2a1 1 0 0 1-1-1m-1 4v3.793l1.146-1.147a.5.5 0 0 1 .708.708l-2 2a.5.5 0 0 1-.708 0l-2-2a.5.5 0 0 1 .708-.708L7.5 11.293V7.5a.5.5 0 0 1 1 0"/>
        ...
        const tableWrapper = document.querySelector('#po_ppic_table_gr_wrapper');
        if (tableWrapper) {
          const existingContainer = document.getElementById('po_ppic_gr_button_container');
          if (existingContainer) existingContainer.remove();
          tableWrapper.insertAdjacentElement('afterend', buttonContainer);
        } else {
          console.error('Table wrapper #po_ppic_table_gr_wrapper not found');
        }
        ...
        setTimeout(function () {
          detailTablePOPPIC.ajax.reload();
        }, 100);

        return detailTablePOPPIC;
      }

      function detail_table_po_approve_ppic() {
        var selectYear = $('#dateSelected').val();

        detailTablePOApprovePPIC = $("#po_approve_table_ppic").DataTable({
          destroy: true,
          processing: true,
          serverSide: true,
          responsive: false,
          ...
          ajax: {
            url: `https://${window.location.host}/api/purchasing/get_data_po_approval_ppic`,
            type: 'POST',
            contentType: "application/json",
            data: function (d) {
              return JSON.stringify({
        ...
        buttonContainer.style.gap = '10px';
        buttonContainer.style.marginTop = '10px';
        buttonContainer.id = 'po_approve_table_ppic_button_container';

        const downloadButton = document.createElement('button');
        downloadButton.id = 'download_po_approve_table_ppic_button';
        downloadButton.className = 'btn btn-primary';
        downloadButton.innerHTML = `
            <svg xmlns="https://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-file-earmark-arrow-down-fill" viewBox="0 0 16 16" style="margin-right: 8px;">
                <path d="M9.293 0H4a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h8a2 2 0 0 0 2-2V4.707A1 1 0 0 0 13.707 4L10 .293A1 1 0 0 0 9.293 0M9.5 3.5v-2l3 3h-2a1 1 0 0 1-1-1m-1 4v3.793l1.146-1.147a.5.5 0 0 1 .708.708l-2 2a.5.5 0 0 1-.708 0l-2-2a.5.5 0 0 1 .708-.708L7.5 11.293V7.5a.5.5 0 0 1 1 0"/>
        ...
        const tableWrapper = document.querySelector('#po_approve_table_ppic_wrapper');
        if (tableWrapper) {
          const existingContainer = document.getElementById('po_approve_table_ppic_button_container');
          if (existingContainer) existingContainer.remove();
          tableWrapper.insertAdjacentElement('afterend', buttonContainer);
        } else {
          console.error('Table wrapper #po_approve_table_ppic_wrapper not found');
        }

        downloadButton.addEventListener('click', function () {
          createDateRangePopup('po_approve_table_ppic');
        });

        setTimeout(function () {
          detailTablePOApprovePPIC.ajax.reload();
        }, 100);

        return detailTablePOApprovePPIC;
      }
      ...
      function createDateRangePopup(tableType) {
        ...
        document.getElementById('popupDownloadBtn').addEventListener('click', function () {
          ...
          if (tableType === 'po_ppic') {
            downloadPOData(startDate, endDate);
          } else if (tableType === 'po_ppic_ammount') {
            downloadPOAmountData(startDate, endDate);
          } else if (tableType === 'po_approve_table_ppic') {
            downloadPOApproveData(startDate, endDate);
          }
          overlay.remove();
        });
      }
      ...
      function downloadPOData(startDate, endDate) {
        ...
        const apiUrl = `https://${window.location.host}/api/purchasing/get_purchase_po_ppic_export`;
        const postData = {
          startDate: startDate,
          endDate: endDate
        };
        ...
        axios.post(apiUrl, postData)
          .then(response => {
            console.log('API Response (po_ppic):', response);
            const tableData = response.data.data;
            if (tableData && tableData.length > 0) {
              const excelContent = convertToExcel(tableData);
              const filename = `po_ppic_data_${startDate}_to_${endDate}.xlsx`;
          ...
          .catch(error => {
            console.error('Error fetching data (po_ppic):', error);
      ...
      function downloadPOApproveData(startDate, endDate) {
        const loadingDiv = document.createElement('div');
        loadingDiv.id = 'loadingOverlay';
        loadingDiv.className = 'fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center';
        loadingDiv.style.zIndex = '10000';
        ...
        const apiUrl = `https://${window.location.host}/api/purchasing/get_data_po_approval_ppic_export`;
        const postData = {
          startDate: startDate,
          endDate: endDate
        };
        ...
        axios.post(apiUrl, postData)
          .then(response => {
            console.log('API Response (po_approve_table_ppic):', response);
            const tableData = response.data.data;
            if (tableData && tableData.length > 0) {
              const excelContent = convertToExcel(tableData);
              const filename = `po_approve_table_ppic_${startDate}_to_${endDate}.xlsx`;
              downloadExcel(excelContent, filename);
            } else {
              alert('No data available for the selected date range.');
            }
          ...
          .catch(error => {
            console.error('Error fetching data (po_approve_table_ppic):', error);
            console.error('Error details:', error.response);
            alert('Failed to fetch data for download.');
          })
          .finally(() => {
      ...
      function downloadPOAmountData(startDate, endDate) {
        ...
        const apiUrl = `https://${window.location.host}/api/purchasing/get_purchase_data_po_ppic_ammount_export`;
        const postData = {
          startDate: startDate,
          endDate: endDate
        };
        ...
        axios.post(apiUrl, postData)
          .then(response => {
            console.log('API Response (po_ppic_ammount):', response);
            const tableData = response.data.data;
            if (tableData && tableData.length > 0) {
              const excelContent = convertToExcel(tableData);
              const filename = `po_ppic_ammount_data_${startDate}_to_${endDate}.xlsx`;
              downloadExcel(excelContent, filename);
            } else {
              alert('No data available for the selected date range.');
            }
          ...
          .catch(error => {
            console.error('Error fetching data (po_ppic_ammount):', error);
            console.error('Error details:', error.response);
            alert('Failed to fetch data for download.');
          })
          .finally(() => {
      ...
      function downloadPOGRData(status) {
        ...
        const apiUrl = `https://${window.location.host}/api/purchasing/get_data_purchasing_po_ppic_table_export`;
        const postData = {
          status: status
        };
        ...
        axios.post(apiUrl, postData)
          .then(response => {
            console.log('API Response (po_ppic_gr):', response);
            const tableData = response.data.data;
            if (tableData && tableData.length > 0) {
              const excelContent = convertToExcel(tableData);
              const filename = `po_ppic_gr_data_${status.replace(/\s+/g, '_')}.xlsx`;
              downloadExcel(excelContent, filename);
            } else {
          ...
          .catch(error => {
            console.error('Error fetching data (po_ppic_gr):', error);
            console.error('Error details:', error.response);
            alert('Failed to fetch data for download.');
          })
          .finally(() => {
      ...
      function setDefaultData() {
        ...
        const apiUrlDaily = `https://${currentHost}/api/purchasing/get_data_purchase_po_ppic`;
        ...
        const apiUrlReceipt = `https://${currentHost}/api/purchasing/get_data_po_ppic_receipt/${selectMonth}`;
        ...
        const apiUrlAging = `https://${currentHost}/api/purchasing/get_data_po_ppic_aging`;
        ...
        const apiApproval = `https://${currentHost}/api/purchasing/get_data_po_ppic_approval/${date}`;
      ...
      detail_table_po_approve_ppic();
      detail_table_po_ppic();
      setDefaultData();
            ...
            chart: {
              ...
              events: {
                dataPointSelection: (event, chartContext, config) => {
                  ...
                  if (detailTablePOPPIC) {
                    detailTablePOPPIC.ajax.reload();
                  } else {
                    console.error('detailTablePOPPIC is not initialized');
                  }
                }
              }
            },
