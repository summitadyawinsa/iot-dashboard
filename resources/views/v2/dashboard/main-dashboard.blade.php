<x-layout.default>
            ...
            <div class="grid lg:grid-cols-4 md:grid-cols-1 lg:gap-3 d:gap-2 mb-3">
                ...
                <div
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-1 gap-4">
                        ...
                        <canvas id="planVsActualChart" class="mx-auto mt-4 text-black dark:text-white"
                ...
                <div
                    ...
                    <div class="flex flex-1 justify-around items-stretch">
                        ...
                        <div class="flex flex-col justify-between items-center w-full px-2">
                            <div class="text-black dark:text-white lg:text-3xl md:text-xl font-semibold"
                                id="std_gsph_title"></div>
                            <div class="flex-grow flex items-center justify-center">
                                <div class="font-bold" style="color: #4ade80; font-size: 5vw;" id="std_gsph">
                                </div>
                            </div>
                            <div class="text-black dark:text-white text-base text-center">
                        ...
                        <div class="flex flex-col justify-between items-center w-full px-2">
                            <div class="text-black dark:text-white lg:text-3xl md:text-xl font-semibold"
                                id="act_gsph_title"></div>
                            <div class="flex-grow flex items-center justify-center">
                                <div class="font-bold text-black dark:text-white" style="font-size: 5vw;"
                                    id="act_gsph"></div>
                            </div>
                            <div class="text-black dark:text-white text-base text-center">
            ...
            <div class="grid lg:grid-cols-4 md:grid-cols-1 lg:gap-3 d:gap-2 mb-2">
                <div
                    ...
                    <div class="flex justify-around">
                        <div class="text-center font-bold text-black dark:text-white text-2xl">
                            <div>A</div>
                        ...
                        <div class="text-center text-black dark:text-white text-2xl font-bold">
                            <div>Q</div>
                    ...
                    <div class="flex justify-between mt-2">
                        <div>
                            <div>Qty Plan</div>
                            <span class="text-black dark:text-white text-xl" id="qty_plan_span"></span>
                        </div>
                        <div>
                            <div>Qty Act</div>
                            <span class="text-black dark:text-white text-xl" id="qty_span"></span>
                        </div>
                        <div>
                            <div>Opr Time</div>
                            <span class="text-black dark:text-white text-xl" id="opr_time_span"></span>
                        </div>
                    </div>
                    <div class="flex justify-between mt-2">
                        <div>
                            <div>DT Duration</div>
                            <span class="text-black dark:text-white text-xl" id="downtime_span"></span>
                        </div>
                        <div>
                            <div>OPR - DT</div>
                            <span class="text-black dark:text-white text-xl" id="opr_dt_span"></span>
                        </div>
                        <div>
                            <div>CT</div>
                            <span class="text-black dark:text-white text-xl" id="ct_span"></span>
                        </div>
                    </div>
                </div>
                <div
    ...
    <script>
        ...
        socket.onmessage = (event) => {
            try {
                ...
                const actCycle = parseFloat(response.act_cycletime).toFixed(2)
                ...
                if (eventTitle === 'update_stroke') {
                    ...
                    const qualityValue = parseFloat(response.ooe_quality) || 0;
                    document.getElementById('quality').textContent = `${qualityValue.toFixed(2)}%`;
                    const availabilityValue = parseFloat(response['oee-availability']) || 0;
                    document.getElementById('availability').textContent = `${availabilityValue.toFixed(2)}%`;
                    const performanceValue = parseFloat(response.oee_performance) || 0;
                    document.getElementById('performance').textContent = `${performanceValue.toFixed(2)}%`;
                    const oeePercentageValue = parseFloat((qualityValue + availabilityValue + performanceValue) / 3)
                        .toFixed(0)
                    document.getElementById('qty_span').textContent = response.qty_actual || 0
                    const opr = response.oprMenit || 0
                    document.getElementById('opr_time_span').textContent = opr.toFixed(2) + 'M'
                    const totalDT = response.dtDuration || 0
                    document.getElementById('downtime_span').textContent = totalDT.toFixed(2) + 'M'
                    const opr_dt = response.oprTimeDT || 0
                    document.getElementById('opr_dt_span').textContent = opr_dt.toFixed(2)
                    const ct_ = response.ctMenit || 0
                    document.getElementById('ct_span').textContent = ct_.toFixed(1)
                    const gsphLabelsObj = response.gsphLabel || {};
                    const gsphValuesArray = response.gsphValues || [];
                    const gsphLabelsArray = Object.keys(gsphLabelsObj)
                        .sort((a, b) => a - b)
                        .map(h => h.padStart(2, '0') + ':00');

                    const gsphValues = gsphLabelsArray.map(label => {
                        const hour = parseInt(label.split(':')[0], 10);
                        return parseFloat(gsphLabelsObj[hour]) || 0;
                    });
                    if (window.gsphMonitoringChartInstance) {
                        window.gsphMonitoringChartInstance.data.labels = gsphLabelsArray;
                        window.gsphMonitoringChartInstance.data.datasets[0].data = gsphValues;
                        window.gsphMonitoringChartInstance.update();
                    }
                }
                if (eventTitle === 'start-downtime' || eventTitle == 'finish-downtime') {
                ...
                } else if (eventTitle == 'start-machine' || eventTitle == 'finish-machine') {
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            const url = `https://${window.location.host}/api/dashboard/${machine_id}`;
            ...
            fetch(url)
                ...
                .then(data => {
                    ...
                    const qtyPlan = data.machine.qty_plan
                    const qtyAct = data.machine.qty_actual
                    document.getElementById('qty_plan_span').textContent = qtyPlan
                    document.getElementById('qty_span').textContent = qtyAct
                    const opr = data.opr_time
                    document.getElementById('opr_time_span').textContent = opr.toFixed(2) + 'M'
                    const totalDT = data.total_dt
                    document.getElementById('downtime_span').textContent = totalDT.toFixed(2) + 'M'
                    const opr_dt = data.opr_dt
                    document.getElementById('opr_dt_span').textContent = opr_dt.toFixed(2)
                    const ct_ = data.ct
                    document.getElementById('ct_span').textContent = ct_.toFixed(1)
                    const machine = data.machine;
                    const robotName = machine.machine_code.split('-').pop()
                    ...
                    document.getElementById('availability').textContent = `${availability.toFixed(2)}%`;
                    document.getElementById('performance').textContent = `${performance.toFixed(2)}%`;
                    document.getElementById('quality').textContent = `${quality.toFixed(2)}%`;
                    ...
                    if (!window.gsphMonitoringChartInstance) {
                        const gsphLabelsArray = Object.keys(data.gsphLabels)
                            .sort((a, b) => a - b)
                            .map(h => h.padStart(2, '0') + ':00');

                        const gsphValuesArray = gsphLabelsArray.map(label => {
                            const hour = parseInt(label.split(':')[0], 10);
                            return data.gsphLabels[hour] ?? 0;
                        });
                        window.gsphMonitoringChartInstance = new Chart(gsphMonitoringCtx, {
                            type: 'bar',
                            data: {
                                labels: gsphLabelsArray,
                                datasets: [{
                                    label: 'GSPH Monitoring',
                                    data: gsphValuesArray,
                                    backgroundColor: '#4ade80',
                                    'maxBarThickness': 50
                                }]
                            },
